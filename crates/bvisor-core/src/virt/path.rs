...
//!   /sys, /run, /.b, most of /dev, /tmp/.bvisor      -> block (EPERM/EACCES)
...
fn route_normalized(path: &str) -> RouteResult {
    ...
    if let Some(rest) = matches_prefix(path, "/tmp") {
        if rest == ".bvisor" || rest.starts_with(".bvisor/") {
            return Block;
        }
        return Handle(Tmp);
    }
...
mod tests {
    ...
    #[test]
    fn bvisor_dir_blocked() {
        assert_eq!(r("/tmp/.bvisor"), Block);
        assert_eq!(r("/tmp/.bvisor/secret"), Block);
        assert_eq!(r("/tmp/.bvisor/sb/uid/cow/etc/passwd"), Block);
    }
    ...
    fn path_traversal_normalized_before_routing() {
        ...
        assert_eq!(r("/tmp/.bvisor/../foo.txt"), Handle(Tmp));
