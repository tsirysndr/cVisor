//! C ABI for bVisor, consumed by the Bun / Deno / Python / Ruby FFI SDKs.
//!
//! A single shared library (`libbvisor.so`) exposes a small set of `extern "C"`
...
//!   BvisorOutput*  bvisor_run(BvisorSandbox*, const char* cmd);
...
//!
//! `bvisor_run` blocks until the guest command exits, then the captured
...
mod imp {
    ...
    use bvisor_core::{cleanup_overlay, execute, generate_uid, LogBuffer, LogLevel};
    ...
    #[no_mangle]
    pub extern "C" fn bvisor_sandbox_new() -> *mut Sandbox {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn bvisor_sandbox_free(sb: *mut Sandbox) {
        if sb.is_null() {
            return;
        }
        // SAFETY: `sb` was produced by bvisor_sandbox_new (Box::into_raw).
        let sb = Box::from_raw(sb);
        cleanup_overlay(&sb.uid);
    }

    #[no_mangle]
    pub unsafe extern "C" fn bvisor_sandbox_set_log_level(sb: *mut Sandbox, level: c_int) {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn bvisor_run(sb: *mut Sandbox, cmd: *const c_char) -> *mut Output {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn bvisor_output_free(out: *mut Output) {
        if !out.is_null() {
            // SAFETY: `out` came from bvisor_run (Box::into_raw).
            drop(Box::from_raw(out));
        }
    }

    #[no_mangle]
    pub unsafe extern "C" fn bvisor_output_stdout(
        out: *mut Output,
        out_len: *mut usize,
    ) -> *mut u8 {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn bvisor_output_stderr(
        out: *mut Output,
        out_len: *mut usize,
    ) -> *mut u8 {
    ...
    /// pointer to free with bvisor_bytes_free (NULL/0 when empty).
    ...
    #[no_mangle]
    pub unsafe extern "C" fn bvisor_bytes_free(ptr: *mut u8, len: usize) {
