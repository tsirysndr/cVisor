//! Per-sandbox overlay root at `/tmp/.cvisor/sb/{uid}` with `cow/` and `tmp/`
...
impl OverlayRoot {
    ...
    pub fn new(uid: [u8; 16]) -> std::io::Result<OverlayRoot> {
        ...
        let root_path = format!("/tmp/.cvisor/sb/{uid_str}");
...
mod tests {
    ...
    fn root_and_subdirs_created() {
        ...
        assert!(o.root_path().ends_with("/tmp/.cvisor/sb/ovtestovtest0001"));
