 ...
 *
 * Exposes a single NIF, cvisor:run_nif/1, which creates a sandbox, runs a
 * command to completion, and returns {ok, Stdout, Stderr} | {error, Reason}.
 * libcvisor.so is dlopen'd at load time (CVISOR_LIB env override, else the
...
static int load_lib(void) {
    ...
    p_run = (fn_run)dlsym(g_lib, "cvisor_run");
