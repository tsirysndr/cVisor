%% bVisor Erlang SDK — a NIF-backed wrapper over the libbvisor C ABI.
%%
%%   {ok, Stdout, Stderr} = bvisor:run(<<"echo hello">>).
%%   %% Stdout = <<"hello\n">>
%%
%% Linux-only. The NIF dlopen's libbvisor.so; set BVISOR_LIB to override the
...
-define(APPNAME, bvisor).
-define(LIBNAME, "bvisor_nif").
